...
void savee(ennemis ennemi,perso perso,vie vie,int saut,SDL_Rect camera)
{
FILE *f;
f=fopen("save.bin","wb");
saves saves;
...
void load(int lvl,ennemis *ennemi,perso *perso,vie *vie,int *saut,SDL_Rect *camera,int *continuer,int *save)
{
if (lvl)
{
FILE *f;
...
int d=0,q=0,z=0,s=0,e=0,jump=0,x=0,y=1;
...
//SDL_LockSurface;
load(lvl,&ennemi,&perso,&vie,&saut,&camera,&continuer,save);
while (continuer)
{
...
switch(event.type)
    {
        case SDL_QUIT:
	savee(ennemi,perso,vie,saut,camera);
            continuer = 0;
	...
	case SDL_KEYDOWN:
            ...
            {
 		case SDLK_ESCAPE:
		//menuingame(&continuer,ennemi,perso,vie,saut,camera,ecran);
...
camera = scrolling(d,q,z,s,camera,perso,escalier);
camera = collision(camera,perso);
perso=mouvement(perso,camera,d,q,z,s,escalier,prevd,prevq,&saut,jump);
...
ennemi=mouvennemi(ennemi,perso,d,q,camera,ecran,&ennmouv,&w,&y);
